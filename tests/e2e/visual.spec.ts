...
test('captures required viewports and verifies the chart canvas is nonblank', async ({ page }, testInfo) => {
  ...
  await chart.hover({ position: { x: 190, y: 150 } })
  const tooltip = page.locator('.market-chart-tooltip')
  await expect(tooltip).toBeVisible()
  await expect(tooltip).toContainText('北京时间')
  ...
  await expect(page.locator('.market-chart-wrap')).toHaveAttribute('data-annotation-count', '2')
  ...
  await page.screenshot({ path: path.resolve('test-results', 'visual', 'replay-answer-412x915.png'), fullPage: true })
