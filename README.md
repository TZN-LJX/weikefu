...
面向手机和电脑浏览器的单人学习工具。前 14 个知识单元按顺序解锁，每个标准单元包含三个步骤：

1. 最多 5 道到期错题回顾。
2. 10 道原书单选题，答对至少 8 道通过。
3. 1 个真实 Binance Futures `ETHUSDT` 历史回放，选择上涨、下跌或震荡。

第 15 单元“真实案例集训”从一开始即可进入，无需先完成前 14 个单元。集训包含 100 个不重复的 Binance Futures 历史案例，其中 `ETHUSDT` 和 `BTCUSDT` 各 50 个；首次进入时随机排序并保存进度，答错会进入错题库但仍继续下一题。
...
公开仓库和 GitHub Pages 只包含应用代码。原始 PDF、原书题库、ETH/BTC 历史案例、`.wkf` 学习包和 API 密钥均被 Git 忽略，不会进入公开部署。
